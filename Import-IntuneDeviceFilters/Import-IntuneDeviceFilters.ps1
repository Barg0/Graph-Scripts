# Script version:   2025-06-07 12:00
# Script author:    Barg0
...
$scriptName = "Import-IntuneDeviceFilters"
...
Write-Log "Importing device filters from: $importDir" -Tag "Info"

# Get all JSON files from the import directory
$files = Get-ChildItem -Path $importDir -Filter *.json -File
if ($files.Count -eq 0) {
    Write-Log "No JSON files found in the import directory: $importDir" -Tag "Info"
    Complete-Script -ExitCode 0
}

# Fetch existing filters once
try {
    $existingResponse = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop
    $existingFilters = $existingResponse.value
    Write-Log "Fetched $($existingFilters.Count) existing filters for comparison." -Tag "Info"
} catch {
    Write-Log "Failed to retrieve existing filters: $_" -Tag "Error"
    Complete-Script -ExitCode 1
}
...
# Loop through files and import each filter
foreach ($file in $files) {
    try {
        $filterData = Get-Content -Path $file.FullName -Raw | ConvertFrom-Json

        # Check if filter already exists (match by displayName)
        $existing = $existingFilters | Where-Object { $_.displayName -eq $filterData.displayName }
        if ($null -ne $existing) {
            Write-Log "Skipped (already exists): $($filterData.displayName)" -Tag "Info"
            $skipped++
            continue
        }

        # Build body from clean subset of properties
        $body = @{
            displayName           = $filterData.displayName
            description           = $filterData.description
            platform              = $filterData.platform
            rule                  = $filterData.rule
            roleScopeTags         = $filterData.roleScopeTags
            assignmentFilterManagementType = $filterData.assignmentFilterManagementType
        } | ConvertTo-Json -Depth 3

        Invoke-MgGraphRequest -Method POST -Uri $uri -Body $body -ContentType "application/json" -ErrorAction Stop | Out-Null
        Write-Log "Imported: $($filterData.displayName)" -Tag "Success"
        $imported++

    } catch {
        Write-Log "Failed to import from $($file.Name): $_" -Tag "Error"
        $failed++
    }
}

Write-Log "Import summary: $imported imported, $skipped skipped, $failed failed." -Tag "Info"
Complete-Script -ExitCode 0