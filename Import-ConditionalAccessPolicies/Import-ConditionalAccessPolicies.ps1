...
while (-not $validLicenseSelected) {
    $entraLicense = Read-Host "Which Entra License do you have? Enter '1' for Entra P1 (E3/Business Premium), '2' for Entra P2 (E5) or '3' for Entra P1 + PIM"
    ...
    switch ($entraLicense) {
        ...
        "3" {
            Write-Log "Selected Entra P1 + PIM" -Tag "Info"
            $entraLicense = "P1+PIM"
            $validLicenseSelected = $true
        }
        default {
...
# Normalize once
$entraLicense = $entraLicense.Trim()

# Create group + auth context only for P2 or P1+PIM
if ($entraLicense -in @('P2','P1+PIM')) {
    try {
        $eligibleAdminsGroupId = New-Group -DisplayName 'Conditional Access - Eligible admins'
        Test-AuthenticationContext
    } catch {
        Write-Log "Failed to prepare Eligible Admins group: $($_.Exception.Message)" -Tag 'Error'
        Complete-Script -ExitCode 1
    }
}

# Map license → subfolder
$licenseToFolder = @{
    'P1'     = 'EntraP1'
    'P2'     = 'EntraP2'
    'P1+PIM' = 'EntraP1_PIM'
...
# Resolve folder from license
if (-not $licenseToFolder.ContainsKey($entraLicense)) {
    Write-Log "Unsupported license value: '$entraLicense'." -Tag 'Error'
    Complete-Script -ExitCode 1
}

$importDir = Join-Path -Path $PSScriptRoot -ChildPath $licenseToFolder[$entraLicense]
...
Get-ChildItem -Path $importDir -Filter *.json | ForEach-Object {
    ...
    try {
        ...
        Write-Log "Imported: '$(Split-Path -Leaf $file)'." -Tag "Success"
    } catch {
        Write-Log "Failed to import policy '$(Split-Path -Leaf $file)': $_" -Tag "Error"
    }
}
